 ...
 * Assumption:
 */
static int onenand_dma_wait(struct mtd_info *mtd)
{
	struct onenand_chip *this = mtd->priv;
	void __iomem *CTRL_DMA_BASE = this->base + ONENAND_CTRL_OFFSET;
	u32 status;
...
static int __devinit s5pc110_onenand_probe(struct platform_device *dev)
{
	...
	printk(KERN_DEBUG "OneNAND: s5pc110_onenand_probe - (Virt) Base Address: 0x%08X\n",
...
/* PM Support */
#ifdef CONFIG_PM
static int s5pc110_onenand_suspend(struct platform_device *pdev, pm_message_t pm)
{
...
static int s5pc110_onenand_resume(struct platform_device *pdev)
{
	struct onenand_info *info = dev_get_drvdata(&pdev->dev);
...
static int __devexit s5pc110_onenand_remove(struct platform_device *dev)
{
	//struct platform_device *pdev = to_platform_device(dev);
	struct platform_device *pdev = dev;
	struct onenand_info *info = dev_get_drvdata(&pdev->dev);
...
static struct platform_driver s5pc110_onenand_driver = {
	.probe		= s5pc110_onenand_probe,
	.remove		= __devexit_p(s5pc110_onenand_remove),
	.suspend	= s5pc110_onenand_suspend,
	.resume		= s5pc110_onenand_resume,
	.shutdown	= NULL,		// TO DO
	.driver		= {
		.name	= DRIVER_NAME,
		.owner	= THIS_MODULE,
...
static int __init s5pc110_onenand_init(void)
{
	printk(KERN_INFO "S5PC110 OneNAND Driver.\n");

	return platform_driver_register(&s5pc110_onenand_driver);
}

static void __exit s5pc110_onenand_exit(void)
{
	platform_driver_unregister(&s5pc110_onenand_driver);
}

module_init(s5pc110_onenand_init);
module_exit(s5pc110_onenand_exit);

MODULE_ALIAS(DRIVER_NAME);
MODULE_LICENSE("GPL");
MODULE_AUTHOR("DongJin Park <djpax.park@samsung.com>");
MODULE_DESCRIPTION("Glue layer for OneNAND flash on S5PC110");
