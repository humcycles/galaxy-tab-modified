...
int fimc_s_parm(struct file *file, void *fh, struct v4l2_streamparm *a)
{
	...
	ctrl->capture_mode = a->parm.capture.capturemode; //JWWON 2010.08.16 add sensor state
...
int fimc_streamon_capture(void *fh)
{
	...
	fimc_dbg("%s: camera sensor state: %d (0: preview, 1: capture)\n", __func__, ctrl->capture_mode);

	if(ctrl->capture_mode == V4L2_MODE_PREVIEW){// JWWON 2010.08.16 add sensor state
	...
	if(ctrl->capture_mode == V4L2_MODE_CAPTURE){// JWWON 2010.08.16 add sensor state 
		struct v4l2_control cam_ctrl;
