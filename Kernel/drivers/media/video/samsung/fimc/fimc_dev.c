...
#ifdef CONFIG_CPU_FREQ
	if (0 == ctrl->id)
    		s5pc110_lock_dvfs_high_level(DVFS_LOCK_TOKEN_2, 2); //LEV_200MHZ);
#endif 
...
#ifdef CLEAR_FIMC2_BUFF
	if(2 == ctrl->id){
		printk(KERN_DEBUG "dev:ctrl->mem.base:%x, ctrl->mem.size:%x\n", ctrl->mem.base, ctrl->mem.size);
...
static int fimc_init_global(struct platform_device *pdev)
{
	...
	for (i = 0; i < FIMC_MAXCAMS; i++) {
		...
		/* mout_cam */
		mout_cam = clk_get(&pdev->dev, "mout_cam0");
		if (IS_ERR(mout_cam)) {
			dev_err(&pdev->dev, "%s: MCLK source clock 'mout_cam0' not found\n", __func__);
			clk_put(cam->srclk);
			return -EINVAL;
		}
		...
		if (!clk_get_parent(mout_cam)) {
			dev_err(&pdev->dev, "%s: failed to get mout_cam clk parent \n", __func__);
			clk_put(mout_cam);
			clk_put(cam->srclk);
			clk_put(cam->clk);
			return -EINVAL;
		...
		clk_set_parent(mout_cam, cam->srclk);
...
static int __devinit fimc_probe(struct platform_device *pdev)
{
	...
	/* set parent for mclk */
	if(clk_get_parent(ctrl->clk->parent))
		clk_set_parent(ctrl->clk->parent, srclk);

	/* set rate for mclk */
	if (clk_get_rate(ctrl->clk)) 
