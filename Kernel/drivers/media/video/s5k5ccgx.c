...
//#define S5K5CCGX_TOUCH_AF // VE_GROUP
...
#endif
static int preview_ratio = 16; // VE_GROUP
...
static int s5k5ccgx_check_sensorId(struct v4l2_subdev *sd)
{
	...
	if (gcamera_sensor_back_checked) 
	{
		...
		if (gcamera_sensor_back_type == CAMERA_SENSOR_ID_S5K5CCGX)
			return 0;
		else
			return -EIO;
	}
...
static int s5k5ccgx_set_frame_rate(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch(ctrl->value)
	{
		...
		case 12:
			 err = s5k5ccgx_i2c_write_block(sd,S5K5CCGX_12_FPS,
...
#ifdef S5K5CCGX_FLASH_SUPPORT
	// Flash off
	err = s5k5ccgx_set_flash_mode(sd,0,false);
	flash_check = 0;
#endif
...
#ifdef S5K5CCGX_FLASH_SUPPORT
	// Flash on
	err = s5k5ccgx_set_flash_mode(sd,15,true);
	if (flash_check)
	{
		flash_forceon = true;
		msleep(200);	
		err = s5k5ccgx_set_flash_mode(sd,0,true);
		err = s5k5ccgx_set_flash_mode(sd,1,true);
		flash_forceon = false;
	    msleep(200);
...
static int s5k5ccgx_set_sensor_mode(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	{
	#ifdef S5K5CCGX_FLASH_SUPPORT
		s5k5ccgx_set_flash_mode(sd,100,true);
	#endif
	}
...
static int s5k5ccgx_set_iso(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch(ctrl->value)
	{
		...
		msleep(133);		
		if (s5k5ccgx_get_illumination(sd) <= 0x0100 )
			msleep(200);		
		break;
...
{
#define AF_WINDOW_WIDTH 1024
#define AF_WINDOW_HEIGHT 1024
#define AF_OUTER_WINDOW_WIDTH 0x140 //(320)
#define AF_OUTER_WINDOW_HEIGHT 0x10A //(266)

#define AF_INNER_WINDOW_WIDTH 0x8F //(143)
#define AF_INNER_WINDOW_HEIGHT 0x8F //(143)
	...
	if(ctrl->value == 1) // Touch AF start
	{
		...
		int outstartX, outstartY;
		int incenterX , incenterY ;
		int insizeX = AF_INNER_WINDOW_WIDTH  , insizeY = AF_INNER_WINDOW_HEIGHT;
		int outcenterX , outcenterY ;
		int outsizeX = AF_OUTER_WINDOW_WIDTH , outsizeY = AF_OUTER_WINDOW_HEIGHT;
		...
		outcenterX = incenterX = (AF_WINDOW_WIDTH* state->position.x)/preview_width;
		outcenterY = incenterY = (AF_WINDOW_HEIGHT*state->position.y)/preview_height;

		//  window  center   center  window  center  
		if (incenterX < (AF_INNER_WINDOW_WIDTH/2)) // less
		{
			incenterX = (AF_INNER_WINDOW_WIDTH/2);
			insizeX = AF_INNER_WINDOW_WIDTH;
		}
		else 
		if ((incenterX+(AF_INNER_WINDOW_WIDTH/2)) > AF_WINDOW_WIDTH) // over
		{
			incenterX = AF_WINDOW_WIDTH-(AF_INNER_WINDOW_WIDTH/2);
			insizeX = AF_INNER_WINDOW_WIDTH;
		}
		
		if (incenterY < (AF_INNER_WINDOW_HEIGHT/2)) // less
		{
			incenterY = (AF_INNER_WINDOW_HEIGHT/2);
			insizeY = AF_INNER_WINDOW_HEIGHT;
		}
		else
		if ((incenterY+(AF_INNER_WINDOW_HEIGHT/2)) > AF_WINDOW_HEIGHT) // over
		{
			incenterY = AF_WINDOW_HEIGHT-(AF_INNER_WINDOW_HEIGHT/2);
			insizeY = AF_INNER_WINDOW_HEIGHT;
		}

		////////////////////// OUT WINDOW ////////////////////////////////
		//  window  center   center  window  center  
		if (incenterX < (AF_OUTER_WINDOW_WIDTH/2)) // less
		{
			outsizeX = (AF_OUTER_WINDOW_WIDTH-incenterX);
			outcenterX = outsizeX/2;
		}
		else 
		if ((incenterX+(AF_OUTER_WINDOW_WIDTH/2)) > AF_WINDOW_WIDTH) // over
		{
			outsizeX =  (AF_OUTER_WINDOW_WIDTH-(AF_WINDOW_WIDTH - incenterX));
			outcenterX = AF_WINDOW_WIDTH - outsizeX/2;
		}
		
		if (incenterY < (AF_OUTER_WINDOW_HEIGHT/2)) // less
		{
			outsizeY = (AF_OUTER_WINDOW_HEIGHT-incenterY);
			outcenterY = outsizeY/2;
		}
		else 
		if ((incenterY+(AF_OUTER_WINDOW_HEIGHT/2)) > AF_WINDOW_HEIGHT) // over
		{
			outsizeY =  (AF_OUTER_WINDOW_HEIGHT-(AF_WINDOW_HEIGHT - incenterY));
			outcenterY = AF_WINDOW_HEIGHT - outsizeY/2;
		}

		instartX = incenterX - (insizeX/2);
		instartY = incenterY - (insizeY/2);
		outstartX = outcenterX - (outsizeX/2);
		outstartY = outcenterY - (outsizeY/2);

		printk("====== PREVIEW (%d,%d) TOUCH (%d,%d) ====\n",preview_width , preview_height , state->position.x ,state->position.y);
		printk("= AF IN center(%d,%d) size(%d,%d) start(%d,%d)\n",incenterX, incenterY , insizeX, insizeY, instartX, instartY );
		printk("= AF OUT center(%d,%d) size(%d,%d) start(%d,%d)\n",outcenterX, outcenterY , outsizeX, outsizeY, outstartX, outstartY );
		
		err  = s5k5ccgx_i2c_write(client, 0xFCFC, 0xD000);
		err += s5k5ccgx_i2c_write(client, 0x0028, 0x7000);
		err += s5k5ccgx_i2c_write(client, 0x002A, 0x022C);
		err += s5k5ccgx_i2c_write(client, 0x0F12, outstartX); 		// FirstWinStartX
		err += s5k5ccgx_i2c_write(client, 0x0F12, outstartY); 		// FirstWinStartY
		err += s5k5ccgx_i2c_write(client, 0x0F12, outsizeX); 				// FirstWinSizeX
		err += s5k5ccgx_i2c_write(client, 0x0F12, outsizeY); 				// FirstWinSizeY
		err += s5k5ccgx_i2c_write(client, 0x0F12, instartX);		// SecondWinStartX
		err += s5k5ccgx_i2c_write(client, 0x0F12, instartY);		// SecondWinStartY
		err += s5k5ccgx_i2c_write(client, 0x0F12, insizeX); 				// SecondWinSizeX
		err += s5k5ccgx_i2c_write(client, 0x0F12, insizeY); 				// SecondWinSizeY
	...
	{
		int outX = (1024/2) - (AF_OUTER_WINDOW_WIDTH/2);
		int outY = (1024/2) - (AF_OUTER_WINDOW_HEIGHT/2);
		int inX = (1024/2) - (AF_INNER_WINDOW_WIDTH/2);
		int inY = (1024/2) - (AF_INNER_WINDOW_HEIGHT/2);
		
		err  = s5k5ccgx_i2c_write(client, 0xFCFC, 0xD000);
		err += s5k5ccgx_i2c_write(client, 0x0028, 0x7000);
		err += s5k5ccgx_i2c_write(client, 0x002A, 0x022C);
		err += s5k5ccgx_i2c_write(client, 0x0F12, outX);    // FirstWinStartX
		err += s5k5ccgx_i2c_write(client, 0x0F12, outY);    // FirstWinStartY
		err += s5k5ccgx_i2c_write(client, 0x0F12, AF_OUTER_WINDOW_WIDTH);    // FirstWinSizeX
		err += s5k5ccgx_i2c_write(client, 0x0F12, AF_OUTER_WINDOW_HEIGHT);    // FirstWinSizeY
		err += s5k5ccgx_i2c_write(client, 0x0F12, inX);    // SecondWinStartX
		err += s5k5ccgx_i2c_write(client, 0x0F12, inY);    // SecondWinStartY
		err += s5k5ccgx_i2c_write(client, 0x0F12, AF_INNER_WINDOW_WIDTH);    // SecondWinSizeX
		err += s5k5ccgx_i2c_write(client, 0x0F12, AF_INNER_WINDOW_HEIGHT);    // SecondWinSizeY
		err += s5k5ccgx_i2c_write(client, 0x0F12, 0x0001);    // WindowSizeUpdated
...
static int s5k5ccgx_get_framesize_index(struct v4l2_subdev *sd)
{
	...
	for(i = 0; i < (sizeof(s5k5ccgx_framesize_list)/sizeof(struct s5k5ccgx_enum_framesize)); i++)
	{
		...
		else 
		{
			 ...
			 * the default rate (currently S5K5CCGX_PREVIEW_WVGA).
			 */ 	 
			if(frmsize->width == state->pix.width && frmsize->height == state->pix.height)
			{
	...
	if (preview_ratio >= 15)
	{
		return (state->oprmode == S5K5CCGX_OPRMODE_IMAGE) ? S5K5CCGX_CAPTURE_W2MP : S5K5CCGX_PREVIEW_WSVGA;
	}
	else
	{
		/* If it fails, return the default value. */
		return (state->oprmode == S5K5CCGX_OPRMODE_IMAGE) ? S5K5CCGX_CAPTURE_3MP : S5K5CCGX_PREVIEW_SVGA;
	}
...
static int s5k5ccgx_set_framesize_index(struct v4l2_subdev *sd, unsigned int index)
{
	...
	for(i = 0; i < (sizeof(s5k5ccgx_framesize_list)/sizeof(struct s5k5ccgx_enum_framesize)); i++)
	{
		if(s5k5ccgx_framesize_list[i].index == index){
			state->framesize_index = s5k5ccgx_framesize_list[i].index; // VE_GROUP
...
#if defined (CONFIG_TARGET_LOCALE_EUR) || defined (CONFIG_TARGET_LOCALE_VZW) || defined (CONFIG_TARGET_LOCALE_HKTW) || defined (CONFIG_TARGET_LOCALE_HKTW_FET) || defined(CONFIG_TARGET_LOCALE_USAGSM)
			if(HWREV >= 16)
#else
			if(HWREV >= 14)
#endif
			{
				printk("This model have FLASH(%d)\n", HWREV);
				flash_mode = ctrl->value;
			}
			else 
			{
				printk("This model have not FLASH(%d)\n", HWREV);
				flash_mode = 1;//always off
			}
			printk("flash mode = %d\n", flash_mode);
			break;
