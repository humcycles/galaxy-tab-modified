...
{
    INC_INT8	acBuffNo1[XPAD_BUFF_MAX];
    INC_INT8	acBuffNo2[XPAD_BUFF_MAX];
    INC_INT8	acBuffNo3[XPAD_BUFF_MAX];
}ST_XPAD_INFO;



#define MAX_HANGUL_BUFF	2351
static unsigned int g_unWansung[MAX_HANGUL_BUFF] = {
...
static unsigned int g_unJohap[MAX_HANGUL_BUFF] = {
...
INC_UINT8 UINCODE2WANSUNG(INC_INT8* pData, INC_UINT16 UCODE)
{
    INC_UINT16 nLoop, unPos = MAX_HANGUL_BUFF;
    ...
    if(unPos >= MAX_HANGUL_BUFF) return INC_ERROR;
...
{
    INC_INT8	cStrlen, cLoop;
    INC_UINT8	ucSPFlag = 0,  ucDataPos = 0;
    INC_UINT16	wBuf[XPAD_SIZE/2];

    cStrlen = strlen(g_stDls.cTransData);
    if(cStrlen == g_stDls.ucPadCnt) return;

	memset(wBuf, INC_NULL, sizeof(wBuf));
    memcpy(wBuf, (INC_UINT16*)g_stDls.cTransData, g_stDls.ucPadCnt);
...
//  	WideCharToMultiByte(CP_ACP, 0, wBuf, g_stDls.ucPadCnt/2+1, g_stDls.cTransData, g_stDls.ucPadCnt+1, INC_NULL, INC_NULL);
}
...
{
    INC_UINT8	ucXPadInd, ucData, ucLength = 0, ucLoop = 0;
    INC_UINT16	wCrcData, wCrcCheck, wPos = wBitrate-1;

    ST_XPAD_INFO	stXpadInfo;
    int nLoop, nPos;

    if(pucData[wPos] & 0xC0) return INC_ERROR;
    ...
    case 2:		// variable size X-PAD
        ...
        for(ucLoop = 0; ; ){
            ...
            if(++ucLoop == XPAD_FRAME_CHECK) return INC_ERROR;
        }
        for(ucLoop = 0; ; ){
            ucData = pucData[wPos--] & 0x1F;
        ...
        wCrcCheck = INC_PAD_CRC_CHECK(pucData, wPos+1, ucLength+2);
