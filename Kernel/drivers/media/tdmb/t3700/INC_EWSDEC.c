...
/***********************************************************************
INC_EWS_INIT()함수는 반드시 INC_CHANNEL_START()함수 호출전에 호출되어야 한다.                                                     
INC_CHANNEL_START()에 리턴값이  INC_SUCCESS이면48ms 주기로 INC_EWS_FRAMECHECK() 
함수를 호출한다. INC_EWS_FRAMECHECK() == INC_SUCCESS 이면 g_stEWSMsg변수에 
값을 복사하고, INC_EWS_INIT()함수를 호출한다.                                 
