...
{	
	...
	/* apply hysteresis */
	//printk("[%s] original lux = %d\n", __func__, result);	
	result = get_next_level(result);	
	//printk("[%s] representative lux = %d\n", __func__, result);
	return sprintf(buf, "%d\n", result);
}
...
static DEVICE_ATTR(lightsensor_file_illuminance, S_IRUGO, lightsensor_file_illuminance_show,NULL);
...
{		
	...
	if((bh1721_read_value(chip->client, chip->illuminance_data))<0)
		return sprintf(buf, "%d\n", error);
#endif	
	return sprintf(buf, "%d\n", light_enable);
}
