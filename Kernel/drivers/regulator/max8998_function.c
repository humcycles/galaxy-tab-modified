...
#include "task_defs.h"
//static BOOLEAN maxim_ac_connected=FALSE;
NU_TIMER                CHG_Connect_Timer;
static NU_SEMAPHORE     Maxim_PM_Semaphore ;
#endif
...
/*
static struct irqaction s3c_pmic_irq = {
	.name		= "pmic interrupt",
	.flags		= IRQF_SHARED ,
	.handler	= pmic_irq,
};
...
boolean Set_MAX8998_PM_REG(max8998_pm_section_type reg_num, byte value)
{
    ...
    #if 0//ndef BOOTLOADER
    STATUS  status;
    status = NU_Obtain_Semaphore(&Maxim_PM_Semaphore, NU_SUSPEND);
    if(status != NU_SUCCESS)
    {
        printk("Maxim_PM_Semaphore NU_Obtain_Semaphore failed\n");
...
===========================================================================*/
void CHG_Connect_Timer_Expire_Routine(unsigned long  timerid)
{
	//DRV_SendMsg(DRV_PMIC_ISR, 0, 0, 0);
}
...
/*
	...
	CHAR name[8];
	OPTION enable;
	UNSIGNED expirations, id, inital_time, reschedule_time;

	GPIO_Drv_UnsetExtInterrupt(PM_INT_N);
       status = NU_Timer_Information(&CHG_Connect_Timer, name, &enable,
                                    &expirations, &id, &inital_time, &reschedule_time);
	if( status == NU_SUCCESS )
	{
		if(enable == NU_ENABLE_TIMER ){
			status = NU_Control_Timer(&CHG_Connect_Timer, NU_DISABLE_TIMER);
			if( status != NU_SUCCESS )
			{
				ERC_System_Error(status);
			}
	...
	status = NU_Reset_Timer(&CHG_Connect_Timer, CHG_Connect_Timer_Expire_Routine, 500, 0, NU_ENABLE_TIMER);
	if( status != NU_SUCCESS )
	{
		ERC_System_Error(status);
	}
...
void lpm_mode_check(void)
{
	...
	{
		printk("[PMIC] %s : VDCINOK (%d)\n", __func__, Get_MAX8998_PM_REG(VDCINOK_status));
...
===========================================================================*/
unsigned char MaxChgStatus=0;
void MAX8998_IRQ_init(void)
{
	byte irq_mask[4], buff[4];
	int ret;
	//STATUS status;
	...
	if(pmic_read(SLAVE_ADDR_PM, (byte)IRQ1_REG, buff, (byte)4) != PMIC_PASS)
		pr_err("Clear IRQ failed\n");


	pmic_int_mem = ioremap(EXT_INT_0_CON, 0x200);

	if(pmic_int_mem ==  NULL)
		printk( "Error at relase PMIC INT \n");

	writel( readl(pmic_int_mem+0x100) & 0x7F, pmic_int_mem+0x100);
	...
	writel(((readl(pmic_int_mem ) & ~(7 << 28)) | 0 << 28), pmic_int_mem);
...
#ifndef FEATURE_SAMSUNG_LSI
	if(Get_MAX8998_PM_REG(LOBAT2_status))
	{
		//DRV_SendMsg(BATT_LEVEL_CHANGE_SIG, NO_PARAM, NO_PARAM, NO_PARAM);
	}
	else if(Get_MAX8998_PM_REG(LOBAT1_status))
	{
		//DRV_SendMsg(BATT_LEVEL_CHANGE_SIG, NO_PARAM, NO_PARAM, NO_PARAM);
	}
#endif
...
#if 0
    //current_time_ptr->day_of_week  = Day of the week [0..6] Monday through Sunday
    //maxim_rtc_ptr[RTC_DAY]  bit[2:0]=1(Sun)~7(Sat)
    if (current_time_ptr->day_of_week == 6) {
        maxim_rtc_ptr[RTC_DAY] = 1;
    }
    else
    {
        maxim_rtc_ptr[RTC_DAY] = current_time_ptr->day_of_week + 2;
    }
#endif
...
void clk_maxim_rtc_to_julian(maxim_rtc_julian_type *current_time_ptr, byte *maxim_rtc_ptr)
{
    ...
    //current_time_ptr->day_of_week  = Day of the week [0..6] Monday through Sunday
    //maxim_rtc_ptr[RTC_DAY]  bit[2:0]=1(Sun)~7(Sat)
    if(maxim_rtc_ptr[RTC_DAY - RTC_START] == 1)
    {
        current_time_ptr->day_of_week = 6; // Sunday
    }
    else
    {
        current_time_ptr->day_of_week = maxim_rtc_ptr[RTC_DAY - RTC_START] - 2; // Sunday
    }
#endif
...
extern void fsa9480_fake_interrupt(void);
...
#ifndef FEATURE_SAIN_TOUCH
	qt602240_inform_charger_connection(0);	//for touch setting;// kmj_sain
#endif	
#ifdef CONFIG_MHL_SII9234
	acc_TA_check(FALSE);
#endif
...
void maxim_charging_topoff(void)
{
	printk("max8906_charging_topoff\n");
	if(curent_device_type != PM_CHARGER_NULL)
	{
//		s3c_cable_charging();
	}
...
void maxim_low_battery_2nd(void) // 3.4v
{
	//DRV_SendMsg(BATT_LEVEL_CHANGE_SIG, NO_PARAM, NO_PARAM, NO_PARAM);   
	printk("battery voltage is under 3.4V\n");
}
...
EXPORT_SYMBOL(maxim_lpm_chg_status);
...
{
	if(Get_MAX8998_PM_REG(TOPOFFR))
	{
		maxim_charging_control(PM_CHARGER_TA, FALSE); //disable
		maxim_charging_control(PM_CHARGER_USB_CABLE, TRUE); //enable
	}
...
void maxim_charging_control(unsigned int dev_type  , unsigned int cmd)
{
	...
	{
		reg_buff[0] = (0x2 <<5) |(0x1 << 3) |(0x5<<0) ; // CHG_TOPOFF_TH=20%, CHG_RST_HYS=100mV, AC_FCGH= 600mA
