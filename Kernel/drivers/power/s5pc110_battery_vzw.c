...
static int manual_temp_adc = 32000;	// hanapark_P1_VZW (for test)
#endif
		...
		if (start_time_test)
		{
			...
			if ((0*60*1000) < total_time && total_time < (test_interval*60*1000))
			{
				if (!charging_mode_get())
					...
					dev_info(dev, "%s: __HIGH_RECOVER_TEMPERATURE_TEST__total_time:%d   \n", __func__, total_time);
			}
			else if ((test_interval*60*1000) < total_time && total_time < (test_interval*2*60*1000))
			{
				if (!charging_mode_get())
					adc = TEMP_HIGH_BLOCK - 2;
					dev_info(dev, "%s: __HIHG_BLOCK_TEMPERATURE_TEST__total_time:%d   \n", __func__, total_time);
			}
...
static int s3c_get_bat_temp(struct power_supply *bat_ps)
{
...
#ifdef __MANUAL_TEMP_TEST__
	fuelgauge_temp = manual_temp_adc;	// hanapark_P1_VZW (for test)
#endif
	...
	if (fuelgauge_temp >= TEMP_FG_HIGH_BLOCK) {
				...
				health != POWER_SUPPLY_HEALTH_UNSPEC_FAILURE)
		{
			...
			printk("POWER_SUPPLY_HEALTH_OVERHEAT\n");
			update=1;
		}
				...
				health == POWER_SUPPLY_HEALTH_FREEZE)
		{
			...
			printk("POWER_SUPPLY_HEALTH_GOOD\n");
			update=1;
		}
				...
				health != POWER_SUPPLY_HEALTH_UNSPEC_FAILURE)
		{
			...
			printk("POWER_SUPPLY_HEALTH_FREEZE\n");
			update=1;
		}
	}

	if ( (health == POWER_SUPPLY_HEALTH_GOOD &&	// not yet blocked... (meet block temperature during execution!)
		s3c_bat_info.device_state != 0x0) ||
		(s3c_bat_info.bat_info.charging_source == CHARGER_BATTERY) )	// hanapark_P1_VZW (ECIM: P100917-0111) (force good during non-charging)
	{
		// ignore overheat !
                                      ...
                                      char *buf)
{
	...
	case BATT_FG_SOC:
		...
		if (charging_mode_get() && (s3c_bat_info.bat_info.batt_vol < 3400) &&
			(s3c_bat_info.bat_info.level > 90))
			i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
				0);
		else
#endif
...
#if defined ( CONFIG_TARGET_LOCALE_KOR ) || defined ( CONFIG_TARGET_LOCALE_VZW ) 
		/* 2010.08.06 
...
static void s3c_cable_check_status(void)
{
	...
	s3c_get_bat_temp(&s3c_power_supplies[CHARGER_BATTERY]); // hanapark_P1_VZW (ECIM: P100917-0111)
