...
#if defined(WL_IW_USE_ESCAN) && defined (WL_IW_USE_ISCAN)
#error WL_IW_USE_ESCAN and WL_IW_USE_ISCAN are mutually exclusive. Use only one.
#endif
...
#define ISCAN_STATE_ABORTED 0x10
#endif
...
)
{
	...
	WL_TRACE(("%s: SIOCSIWAP - %02X:%02X:%02X:%02X:%02X:%02X\n", 
		dev->name, awrq->sa_data[0], awrq->sa_data[1], awrq->sa_data[2], awrq->sa_data[3], awrq->sa_data[4], awrq->sa_data[5]));
...
)
{
	...
	WL_TRACE(("%s: SIOCGIWAP - %02X:%02X:%02X:%02X:%02X:%02X\n", 
		dev->name, awrq->sa_data[0], awrq->sa_data[1], awrq->sa_data[2], awrq->sa_data[3], awrq->sa_data[4], awrq->sa_data[5]));
...
#endif /* WL_IW_USE_ISCAN || WL_IW_USE_ESCAN */
...
wl_iw_iscan(iscan_info_t *iscan, wlc_ssid_t *ssid, uint16 action)
{
	...
	WL_TRACE(("wl_iw_iscan: ISCAN action=%d\n", action));
...
wl_iw_timerfunc(ulong data)
{
	...
	if (iscan) {
		...
		if (iscan->iscan_state != ISCAN_STATE_IDLE) {
			WL_TRACE(("wl_iw_timerfunc: timer trigger\n"));
			up(&iscan->sysioc_sem);
		}
	}
}
...
static void wl_iw_send_scan_complete(struct net_device *dev)
{
	union iwreq_data wrqu;
...
wl_iw_iscan_get(iscan_info_t *iscan)
{
	...
	WL_TRACE(("wl_iw_iscan_get: results->count = %d, buflen = %d\n", results->count, results->buflen));
...
_iscan_sysioc_thread(void *data)
{
	...
	if  (g_scan_specified_ssid && (iscan_pass_abort == TRUE)) {
		WL_TRACE(("%s Get results from specific scan status=%d\n", __FUNCTION__, status));
			wl_iw_send_scan_complete(iscan->dev);
		...
		switch (status) {
			...
			case WL_SCAN_RESULTS_SUCCESS:
				...
				if (g_assoc_waiting) {
					struct iw_point dwrq;
					dwrq.length = g_ssid_buf_len;
					g_assoc_waiting = 0;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27))
					rtnl_lock();
#endif
					wl_iw_set_essid(iscan->dev, NULL, &dwrq, (char *)g_ssid_buf);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27))
					rtnl_unlock();
#endif
					wl_iw_send_scan_complete(iscan->dev);
			...
			case WL_SCAN_RESULTS_ABORTED:
				...
				if (g_scan_specified_ssid == 0) {
					#ifdef AUTH_TIME_PATCH
					iscan->iscan_state = ISCAN_STATE_ABORTED;
					#endif
					wl_iw_send_scan_complete(iscan->dev);
				}
				else {
...
wl_iw_add_bss_to_ss_cache(wl_scan_results_t *ss_list)
{
	...
	WL_TRACE(("wl_iw_add_bss_to_ss_cache: count=%d\n", ss_list->count));
...
static int
wl_iw_delete_bss_from_ss_cache(uchar *addr)
{

	wl_iw_ss_cache_t *node, *prev;
	wl_iw_ss_cache_t **spec_scan_head;

	WL_TRACE(("wl_iw_delete_bss_from_ss_cache: removing %02X:%02X:%02X:%02X:%02X:%02X\n", 
...
)
{
	...
	if (wrqu->data.length == sizeof(struct iw_scan_req)) {
		if (wrqu->data.flags & IW_SCAN_THIS_ESSID) {
			...
			if (g_scan_specified_ssid) {
				...
				WL_TRACE(("%s: Specific scan ssid='%s' len=%d\n", \
						__FUNCTION__, g_specific_ssid.SSID, g_specific_ssid.SSID_len));
			}
		}
	}
#endif 
	...
	error = wl_iw_escan(g_escan, &g_specific_ssid, WL_SCAN_ACTION_START);
	if (error) {
		WL_ERROR(("%s: Set SCAN for '%s' failed with %d\n", __FUNCTION__, g_specific_ssid.SSID, error));
...
)
{
	...
	if (iscan->iscan_state != 0) {
		WL_ERROR(("%s: scan in progress. state=%d\n", __FUNCTION__, iscan->iscan_state));
		return EBUSY;
	}
...
)
{
	...
	if (escan->escan_state == ESCAN_STATE_SCANING) {
		if ((g_scan_specified_ssid == 0) && (specific_scan != 0)) {
			...
			WL_DEBUG(("%s: Specific scan aborts broad scan. ESCAN abort. err=%d\n", __FUNCTION__, err));
			...
			WL_DEBUG(("%s: Scan in progress. Ignore duplicate scan\n", __FUNCTION__));
			return -EBUSY;
		}
	}
	...
	short max_size)
{
	...
	for (i = 0; i < list->count && i < IW_MAX_AP; i++)
	{
		...
		WL_SCAN(("%s : %02X:%02X:%02X:%02X:%02X:%02X %s\n", 
			__FUNCTION__, bi->BSSID.octet[0], bi->BSSID.octet[1], bi->BSSID.octet[2], 
			bi->BSSID.octet[3], bi->BSSID.octet[4], bi->BSSID.octet[5], (char *)bi->SSID));
		/* check the duplicates in the cache */
        node = g_ss_cache_ctrl.m_cache_head;
        for (;node;) {
            if (!memcmp(&node->bss_info->BSSID,&bi->BSSID,ETHER_ADDR_LEN) && list != (wl_scan_results_t *)node) {
                WL_SCAN(("%s : The scan entry is same with specific cache_entry : SSID => %s\n", __FUNCTION__, (char *)bi->SSID));
                break;
            }
            node = node->next;
        }
...
)
{
	...
	WL_TRACE(("wl_iw_get_scan(%s): buflen_from_user %d: \n", dev->name, buflen_from_user));
...
#ifndef WL_IW_USE_ESCAN
	/* list->version could be 0 in case of ESCAN */
	if (list->version != WL_BSS_INFO_VERSION) {
		WL_ERROR(("%s : list->version %d != WL_BSS_INFO_VERSION\n", \
			 __FUNCTION__, list->version));
		if (g_scan_specified_ssid) {
...
#if defined(WL_IW_USE_ISCAN)
	if (g_scan_specified_ssid)
		WL_SCAN(("%s: Specified scan APs from scan=%d\n", __FUNCTION__, list->count));

	p_buf = iscan->list_hdr;

	while (p_buf != iscan->list_cur) {
		list_merge = &((wl_iscan_results_t*)p_buf->iscan_buf)->results;
		WL_SCAN(("%s: Bcast APs list=%d\n", __FUNCTION__, list_merge->count));
	...
	WL_SCAN(("%s merged with total Bcast APs=%d\n", __FUNCTION__, counter));
#elif defined(WL_IW_USE_ESCAN)
...
#define MAX_SCAN_CACHE_ITEMS 200
...
SCAN_CACHE scan_cache[MAX_SCAN_CACHE_ITEMS];
uint32 scan_cache_count = 0;

#ifdef WL_IW_USE_ISCAN
static void wl_iw_scan_cache_init(iscan_buf_t * p_buf, iscan_info_t *iscan)
#elif WL_IW_USE_ESCAN
static void wl_iw_scan_cache_init(wl_scan_results_t *p_buf)
#endif
{
#ifdef WL_IW_USE_ISCAN
	wl_scan_results_t *list;
	...
	WL_TRACE(("wl_iw_scan_cache_init: %d APs\n", counter));
}

static void wl_iw_scan_cache_add(wl_bss_info_t *bi)
{
	SCAN_CACHE *pCache;
	int i;

	if (scan_cache_count == MAX_SCAN_CACHE_ITEMS)
		return;
	...
	for (i=0, pCache = scan_cache; i<scan_cache_count; i++, pCache++) {
		/* Check duplicates */
		if ((bi->SSID_len == pCache->ssid_len) 
			&& (bi->chanspec == pCache->chanspec) 
			&& (0 == memcmp(&bi->BSSID, pCache->bssid, 6))) {
			/* duplicate found */
			return;
		}
	}

	/* pCache = &scan_cache[scan_cache_count++]; */
	pCache->ssid_len = bi->SSID_len;
	pCache->chanspec = bi->chanspec;
	memcpy(pCache->ssid, bi->SSID, bi->SSID_len);
	memcpy(pCache->bssid, &bi->BSSID, 6);
	pCache->ssid[pCache->ssid_len] = 0;

	scan_cache_count++;

	/* WL_SCAN(("wl_iw_scan_cache_add: %02X:%02X:%02X:%02X:%02X:%02X %04X %s\n",
		pCache->bssid[0], pCache->bssid[1], pCache->bssid[2], pCache->bssid[3], pCache->bssid[4], pCache->bssid[5],
		pCache->chanspec, pCache->ssid)); */
}

#ifdef WL_IW_USE_ESCAN

static void wl_iw_scan_cache_print_list(void);

static void wl_iw_escan_cache_update(escan_info_t *escan)
{
	uint32 i;
	wl_scan_results_t *list = (wl_scan_results_t *)escan->escan_buf;
	wl_bss_info_t *bi = list->bss_info;

	for(i=0; i<list->count; i++) {
		wl_iw_scan_cache_add(bi);
		bi = (wl_bss_info_t *)((uintptr)bi + dtoh32(bi->length));
	}
}
#endif

static int wl_iw_scan_cache_get_channels(chanspec_t *channels)
{
	int i;
	int cnt = 0;
	SCAN_CACHE *pCache = scan_cache;
	chanspec_t *pLast = channels;

	for(i=0; i<scan_cache_count; i++, pCache++)
	{
		if ((g_ssid.SSID_len == pCache->ssid_len)
			&& (0 == memcmp(pCache->ssid, g_ssid.SSID, g_ssid.SSID_len))) {
			chanspec_t *p;

			/* check if same channel setting exists */
			p = channels;
			while(p != pLast) {
				if (*p == pCache->chanspec)
					break;
				++p;
			}
			if (p == pLast) {
// FEATURE.P1_VZW_CDMA by namju - temporary fixation to make connection successful
                        int c = CHSPEC_CHANNEL(pCache->chanspec) ;
                          if ((c > 0) && (c <= 14)) {  // Devices for VZW and Sprint only support 2.4GHz
				*pLast++ = pCache->chanspec;
				cnt++;
					WL_DEBUG(("wl_iw_scan_cache_get_channels: %d %04X\n", cnt, pCache->chanspec));
				}
				else {
					WL_ERROR(("wl_iw_scan_cache_get_channels: error channel = %04X\n", pCache->chanspec));
                          }
			}
		}
	}
...
static void wl_iw_scan_cache_print_list(void)
{
	...
	for(i=0; i<scan_cache_count; i++, pCache++)
	{
		WL_SCAN(("[%d] %02X:%02X:%02X:%02X:%02X:%02X %04X %s\n", i+1,
			pCache->bssid[0], pCache->bssid[1], pCache->bssid[2], pCache->bssid[3], pCache->bssid[4], pCache->bssid[5],
			pCache->chanspec, pCache->ssid));
	}
}
...
)
{
...
#ifdef AUTH_TIME_PATCH
	if (iscan->iscan_state == ISCAN_STATE_ABORTED) {
		WL_SCAN(("wl_iw_scan_get_scan: ISCAN_STATE_ABORTED. skip cache update\n"));
		is_cache_update = FALSE;
		iscan->iscan_state = ISCAN_STATE_IDLE;
	} else {
		wl_iw_scan_cache_init(p_buf, iscan);
	}
#endif
...
#elif defined(WL_IW_USE_ESCAN)
	{
		...
		/* list->version could be 0 when there is no results at this point
	    if (list->version != WL_BSS_INFO_VERSION) {
			WL_ERROR(("%s : list->version %d != WL_BSS_INFO_VERSION\n", \
				 __FUNCTION__, list->version));
			return -EINVAL;
...
#endif
			WL_SCAN(("%s: [%d] %02X:%02X:%02X:%02X:%02X:%02X %04X %s\n", 
				__FUNCTION__, i++, bi->BSSID.octet[0], bi->BSSID.octet[1], bi->BSSID.octet[2],
				bi->BSSID.octet[3], bi->BSSID.octet[4], bi->BSSID.octet[5], bi->chanspec, bi->SSID));
#endif /* AUTH_TIME_PATCH */
	        node = g_ss_cache_ctrl.m_cache_head;
...
)
{
	...
	WL_DEBUG(("%s: SIOCSIWESSID\n", dev->name));
#ifdef WL_IW_USE_ISCAN
	if (g_scan_in_progress) {
		int len = MIN(32, dwrq->length);

		memcpy(g_ssid_buf, extra, len);
		g_ssid_buf[len] = 0;
		g_ssid_buf_len = len;
		WL_TRACE(("%s: Scan in progress. Wait till scan complete\n", __FUNCTION__));
		g_assoc_waiting = 1;
	...
	if (dwrq->length && extra) {
		...
		if (!(join_params = kmalloc(WL_JOIN_PARAMS_MAX,GFP_KERNEL))) {
			...
			WL_ERROR(("wl_iw_set_essid: allocation failed for join_params size is %d\n", WL_JOIN_PARAMS_MAX));
			goto normal_scan;
		}
...
#ifdef WL_IW_USE_ISCAN
		memcpy(g_ssid_buf, g_ssid.SSID, g_ssid.SSID_len);
		g_ssid_buf_len = g_ssid.SSID_len;
		g_ssid_buf[g_ssid_buf_len] = 0;
#endif

		cnt = wl_iw_scan_cache_get_channels(join_params->params.chanspec_list);
		/* Search Scan Cache */
        if(!cnt) {
            MUTEX_LOCK_WL_SCAN_SET();
            node = g_ss_cache_ctrl.m_cache_head;
            ...
            for (;node;) {
                if(!memcmp(&node->bss_info->SSID,join_params->ssid.SSID,join_params->ssid.SSID_len)) {
					join_params->params.chanspec_list[cnt++] = node->bss_info->chanspec;
	                WL_SCAN(("cache_scan : chanspec :%d, count %d\n", (int)node->bss_info->chanspec, cnt));
                }
                node = node->next;
            }
            MUTEX_UNLOCK_WL_SCAN_SET(); 
		...
		if ((cnt > 0) && (join_params_size <= WL_JOIN_PARAMS_MAX)) {
			join_params->ssid.SSID_len = htod32(g_ssid.SSID_len);  
			...
			if ((error = dev_wlc_ioctl(dev, WLC_SET_SSID, join_params, join_params_size))) {
				WL_ERROR(("%s: WLC_SET_SSID failed. error=%d\n", __FUNCTION__, error));
			}
			kfree(join_params);
...
)
{
	...
	WL_TRACE(("%s: SIOCSIWGENIE ID=%d len=%d\n", dev->name, extra[0], iwp->length));
...
#ifdef NEW_AP_INTERFACE
	//cyj_dj15 if (ap_cfg_running == FALSE) {
		updown = 0;
...
#define WLC_BSS_RSSI_ON_CHANNEL 0x0002 
	...
	if (g_scan_specified_ssid) {
		memcpy(&g_scan[list->buflen], bi, bi_length);
	} else {
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	case WLC_E_SCAN_COMPLETE:
		...
		cmd = SIOCGIWSCAN;
		wrqu.data.length = strlen(extra);
		WL_TRACE(("Event WLC_E_SCAN_COMPLETE status=%d len=%d\n", status, wrqu.data.length));
#endif 
		break;
	...
	case WLC_E_SET_SSID:
		...
		/* if (status == WLC_E_STATUS_NO_NETWORKS) {
			struct iw_point dwrq;
			WL_TRACE(("WLC_E_STATUS_NO_NETWORKS: Retry Association to '%s'...\n", g_ssid_buf));
			dwrq.length = g_ssid_buf_len;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27))
			rtnl_lock();
#endif
			wl_iw_set_essid(dev, NULL, &dwrq, (char *)g_ssid_buf);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27))
			rtnl_unlock();
#endif
		}*/
	...
	case WLC_E_ESCAN_RESULT:
		if(g_escan == NULL) {
			WL_ERROR(("Event WLC_E_ESCAN_RESULT: g_escan is NULL \r\n"));
			break;
		}
		WL_SCAN(("Event WLC_E_ESCAN_RESULT: evt status=%d, escan state=%d\n", status, g_escan->escan_state));
		if (g_escan->escan_state == ESCAN_STATE_IDLE) {
			break;
		}
		...
		if (status == WLC_E_STATUS_FAIL) {
			...
			if(g_escan != NULL) {
				if (g_escan->escan_state == ESCAN_STATE_SCANING) {
					...
					WL_ERROR(("Event WLC_E_SET_SSID: Matching network not found\n"));
					/* wl_iw_send_priv_event(g_escan->dev, "ESSID_FAIL"); */
				}
			}
		}
...
#endif 
	case WLC_E_AUTH:
		WL_TRACE(("Event WLC_E_AUTH: status = %d\n", status));
		break;

	default:
		
		WL_TRACE(("wl_iw_event: Unknown Event %d: ignoring\n", event_type));
		break;
	}
#ifndef SANDGATE2G
	if (cmd) {
...
int wl_iw_attach(struct net_device *dev, void * dhdp)
{
	...
	WL_DEBUG(("wl_iw_attach : DHD_USE_STATIC_BUF : use STATIC buffer \r\n"));
	g_escan = (escan_info_t *)dhd_os_prealloc(DHD_PREALLOC_WL_BUF, sizeof(escan_info_t));
#else
	WL_DEBUG(("wl_iw_attach : not use STATIC buffer \r\n"));

	g_escan = (escan_info_t *)kmalloc(sizeof(escan_info_t), GFP_KERNEL);
	if (!g_escan)
		return -ENOMEM;
#endif
	...
	g_scan = (uchar *)kmalloc(G_SCAN_RESULTS, GFP_KERNEL);
	if (!g_scan)
		return -ENOMEM;
