...
#define DHD_PREALLOC_WL_BUF_SIZE    (80*1024)
...
dhd_net_attach(dhd_pub_t *dhdp, int ifidx)
{
	...
	DHD_TRACE(("%s: Broadcom Dongle Host Driver mac=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x\n", net->name,
	       dhd->pub.mac.octet[0], dhd->pub.mac.octet[1], dhd->pub.mac.octet[2],
	       dhd->pub.mac.octet[3], dhd->pub.mac.octet[4], dhd->pub.mac.octet[5]));
...
#if defined(CONFIG_HAS_EARLYSUSPEND)
			if ((dhd->early_suspend.link.next) && (dhd->early_suspend.link.prev))
...
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && defined(CONFIG_PM_SLEEP) */
	/* && defined(DHD_GPL) */
			if (dhdp->wakelock[WAKE_LOCK_TMOUT].name) {
...
void * dhd_os_prealloc(int section, unsigned long size)
{
	...
	switch(section)	{		
		...
		case DHD_PREALLOC_OSL_BUF:			
			...
			break;			
		case DHD_PREALLOC_WL_BUF:
			alloc_ptr =(void *)(static_ptr+DHD_PREALLOC_PROT_SIZE+DHD_PREALLOC_RXBUF_SIZE+DHD_PREALLOC_DATABUF_SIZE+DHD_PREALLOC_OSL_BUF_SIZE);			
			DHD_INFO(("BRCM WIFI VIRTUAL ADDRESS 4 : %x\n", (u32)alloc_ptr));
			break;
	}	
	return (alloc_ptr);
#else
