...
static irqreturn_t s3c_udc_irq(int irq, void *_dev)
{
	...
	if (intr_status & INT_OUT_EP)
		{process_ep_out_intr(dev);goto OK_OUT;}

	if (intr_status & INT_IN_EP)
		{process_ep_in_intr(dev);goto OK_OUT;}
...
static void s3c_ep0_setup(struct s3c_udc *dev)
{
	...
	if (usb_ctrl.bRequestType == 0x21 && usb_ctrl.bRequest == 0) {
		if (tmp_out == 0)
			tmp_out++;
		else {
			printk("\t[%s] invalid tmp_out = %d\n", __func__, tmp_out);			
			s3c_udc_ep0_set_stall(ep);
			dev->ep0state = WAIT_FOR_SETUP;			
			return;
		}
	...
	if (usb_ctrl.bRequestType == 0xa1 && usb_ctrl.bRequest == 0x01) {
		if (tmp_out == 1)
			tmp_out--;
		else {
			printk("\t[%s] invalid tmp_out = %d\n", __func__, tmp_out);			
