...
#define S3C_UDC_DBG(format,...)\
	printk ("[ S3C_UDC_DBG (%s,%d) ] " format "\n", __func__, __LINE__, ## __VA_ARGS__);
...
int s3c_usb_cable(int connected)
{
	...
	printk("s3c_usb_cable: %d\n", connected);

#ifdef CONFIG_USB_S3C_OTG_HOST
	if(atomic_read(&g_OtgHostMode))
		if(connected == USB_CABLE_DETACHED || connected == USB_CABLE_ATTACHED) {
			S3C_UDC_DBG("error!!~~ current mode is OtgHost!!!...\n");
			return 0;
		}
#endif
	...
	switch(connected) {
		...
		case USB_OTGHOST_DETACHED:
			...
			if(atomic_read(&g_OtgHostMode))
			{
				...
				S3C_UDC_DBG("platform_driver_unregister...\n");
			}

			/* irq setup after old hardware state is cleaned up */
			retval = request_irq(IRQ_OTG, s3c_udc_irq, 0, driver_name, dev);
			if (retval != 0) {
				S3C_UDC_DBG("can't get irq %i, err %d\n");
				return -EBUSY;
			}
