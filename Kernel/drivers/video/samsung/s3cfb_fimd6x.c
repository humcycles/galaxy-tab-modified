...
#else
	cfg &= ~(S3C_VIDCON0_CLKSEL_MASK | S3C_VIDCON0_CLKVALUP_MASK | \
		S3C_VIDCON0_VCLKEN_MASK | S3C_VIDCON0_CLKDIR_MASK | (0xFF<<6));
#endif
...
//				(var->bits_per_pixel / 8) * var->yoffset);
		...
		start_addr = fix->smem_start + fix->line_length * var->yoffset;
...
#if defined(CONFIG_MACH_S5PC110_P1) && defined(CONFIG_FB_S3C_MDNIE)
