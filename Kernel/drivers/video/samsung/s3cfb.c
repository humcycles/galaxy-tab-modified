...
#define VERIZON_LOGO  // add for Verizon Model. 
#endif
#ifdef CONFIG_TARGET_LOCALE_USAGSM
#define ATT_LOGO  // add for ATT Model. 
#endif
...
#ifdef CONFIG_TARGET_LOCALE_VZW
extern int s5p_lcd_set_power(struct lcd_device *ld, int power);
#endif
...
#if defined(CONFIG_MACH_S5PC110_P1)
#include "rotate_90_GT_P1100_1200x600.h"
#if defined(CONFIG_TARGET_LOCALE_NTT)
#include "rotate_90_GT_NTT_1200x600.h"
#else
#include "rotate_90_GT_P1000_1200x600.h"
#endif
#include "logo_charging.h"

extern int get_boot_charger_info(void);

#define SDRAM1_BASE_ADDR		0x40000000
#define BOOT_FB_BASE_ADDR			(SDRAM1_BASE_ADDR + 0x0EC00000) /* 0x4EC00000 - hmm.. */

#ifdef VERIZON_LOGO
//#include "rotate_90_SCH_P100_1200x600.h" 
#include "rotate_90_SCH_I800_1200x600.h"
#endif

#ifdef ATT_LOGO
#include "rotate_90_SGH_I987_1200x600.h"
#endif

static int s3cfb_draw_logo(struct fb_info *fb)
{
#ifdef __COPY_LOGO_FROM_BOOTLOADER__
    ...
    int logo_width = 1024;
    int x_offset = (var->xres - logo_width) / 2;
    int y;
	unsigned int * LOGO;
	int charging_mode;
	int logo_size;
...
#ifdef VERIZON_LOGO
    if ( gpio_get_value(GPIO_LCD_ID) || HWREV >= 0x5 )
	    LOGO = LOGO_VZW_I800_RGB24;
#elif defined(ATT_LOGO)
	if ( gpio_get_value(GPIO_LCD_ID) || HWREV >= 0x5 )
		LOGO = LOGO_I897_RGB24;
#else 
	if ( gpio_get_value(GPIO_LCD_ID) || HWREV >= 0x5 )
		LOGO = LOGO_P1000_RGB24;
#endif	
	else
		LOGO = LOGO_P1100_RGB24;

	charging_mode = get_boot_charger_info();
	if(charging_mode)
	{
		logo_width = 624;
        x_offset = (var->xres - logo_width) / 2;
		LOGO = LOGO_CHARGING_RGB24;
		#if 0
		logo_size = logo_width * var->yres * (var->bits_per_pixel / 8);
		remap_fb_ptr = ioremap_nocache(BOOT_FB_BASE_ADDR, logo_size);
		if(remap_fb_ptr)
			{
			LOGO = (unsigned int*)remap_fb_ptr;
			x_offset = 0;
			}
		else
			printk(KERN_ERR "failed to remap logo for charging mode\n");
		#endif
	}

	for(y = 0; y < var->yres; y++)  {
		memcpy(fbdev->fb[pdata->default_win]->screen_base + fix->line_length * y + x_offset * (var->bits_per_pixel / 8),
		LOGO + logo_width * y, logo_width * (var->bits_per_pixel / 8));
	}

	#if 0
	if(charging_mode && remap_fb_ptr)
		iounmap(remap_fb_ptr);
	#endif
	
#endif //__COPY_LOGO_FROM_BOOTLOADER__
        return 0;
}

#else //defined(CONFIG_MACH_S5PC110_P1) 
...
static int s3cfb_sysfs_store_lcd_power(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (strnicmp(buf, "on", 2) == 0 || strnicmp(buf, "1", 1) == 0)
		s5p_lcd_set_power(0,1); 
	else if (strnicmp(buf, "off", 3) == 0 || strnicmp(buf, "0", 1) == 0)
		s5p_lcd_set_power(0,0); 
	else
		return -EINVAL;
			 ...
			 void *data)
{
	...
	case CPUFREQ_PRECHANGE:
		//printk(KERN_INFO "s3cfb cpufreq prechange\n");
		break;

	case CPUFREQ_POSTCHANGE:
		//printk(KERN_INFO "s3cfb cpufreq postchange\n");
		break;
	}
	return 0;
}
...
void s3cfb_late_resume(struct early_suspend *h)
{
	...
	if (pdata->reset_lcd)
		...
		printk("%s: no reset_lcd function\n", __FUNCTION__);
#endif
...
#if defined(CONFIG_FB_S3C_TL2796) || defined(CONFIG_FB_S3C_LVDS)
	if (pdata->backlight_on)
		pdata->backlight_on(pdev);
#elif defined (CONFIG_FB_S3C_LTE480WV)
	if (pdata->backlight_onoff)
