...
#ifndef CONFIG_ANDROID_BUF_NUM	//namkh, test
#define CONFIG_ANDROID_BUF_NUM	4
#endif
...
static void s3c_dma_enqueue(struct snd_pcm_substream *substream)
{
	...
	//	if (s3c_dma_has_circular()) //sayanta ..need to check functionality
...
struct snd_soc_platform s5pc110_soc_platform = {
	.name		= "s5pc110-audio",
	.pcm_ops 	= &s3c_dma_ops,
	.pcm_new	= s3c_dma_new,
	.pcm_free	= s3c_dma_free_dma_buffers,
};
EXPORT_SYMBOL_GPL(s5pc110_soc_platform);
...
#else
	return snd_soc_register_platform(&s5pc110_soc_platform);
#endif
}
...
#else
	snd_soc_unregister_platform(&s5pc110_soc_platform);
#endif
}
