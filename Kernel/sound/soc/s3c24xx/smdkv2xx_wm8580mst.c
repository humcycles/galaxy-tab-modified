	...
	struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_clkdiv(&wm8580_dai[WM8580_DAI_PAIFTX], WM8580_BCLKRATIO, bfs);
	if (ret < 0)
		return ret;

	ret = snd_soc_dai_set_clkdiv(&wm8580_dai[WM8580_DAI_PAIFRX], WM8580_BCLKRATIO, bfs);
	if (ret < 0)
		return ret;
	...
	ret = snd_soc_dai_set_clkdiv(&wm8580_dai[WM8580_DAI_PAIFTX], WM8580_MCLKRATIO, rfs);
	if (ret < 0)
		return ret;

	ret = snd_soc_dai_set_clkdiv(&wm8580_dai[WM8580_DAI_PAIFRX], WM8580_MCLKRATIO, rfs);
	if (ret < 0)
		return ret;
