...
void dvs_set_for_1dot2Ghz (int onoff) {
	...
	if (onoff) {
		max8998_set_dvsarm_direct(DVSARM4, 1350);
		max8998_set_dvsint_direct(DVSINT2, 1125);
	} else {
		max8998_set_dvsarm_direct(DVSARM4, 975);
...
//	max8998_ldo_disable_direct(MAX8998_LDO7);
	...
	max8998_ldo_disable_direct(MAX8998_LDO11);
...
static int s3c_consumer_resume(struct platform_device *dev)
{
	...
	for (i=MAX8998_LDO3;i<=MAX8998_LDO17;i++) {
        ...
        // 20110113 - dukho.kim : control power of moviNAND.
	if(HWREV >= 11) {		
