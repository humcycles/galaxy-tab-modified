...
void set_dvfs_perf_level(void) 
{
	...
	if(s5pc11x_cpufreq_index >= (S5PC11X_MAXFREQLEVEL - 1)) { // for 200, 100 mhz
		if (S5PC11X_FREQ_TAB) 
			s5pc11x_cpufreq_index = 2; 
		else 
...
void s5pc110_lock_dvfs_high_level(unsigned int nToken, unsigned int level) 
{
	...
	//printk(KERN_DEBUG "%s(%d) token=%d\n", __func__, __LINE__, nToken);
	...
	if (g_dvfs_high_lock_token & (1 << nToken) ) printk (KERN_DEBUG "\n\n[DVFSLOCK] lock token %d is already used!\n\n", nToken);
...
//	printk("s5pc110_target called for freq=%d, %d\n",target_freq, S5PC11X_FREQ_TAB);
...
#if 1 // MIDAS[2010.07.28]@Apply DVFS threshold table
...
EXPORT_SYMBOL(dvfs_class);
static struct device *dvfs_fs;
EXPORT_SYMBOL(dvfs_fs);

extern void max8998_init(void);

static void s5pc110_change_freq_tab(unsigned int freq_tab)
{
	int i;
	struct cpufreq_policy *policy;
	
	if (freq_tab > 100 || freq_tab < 0)
		return -EINVAL;

	/*Fix the upper transition scaling*/
	g_dvfs_fix_lock_limit = true;
	s5pc110_lock_dvfs_high_level(DVFS_LOCK_TOKEN_5, 1);

//	mutex_lock(&dbs_mutex);
	S5PC11X_FREQ_TAB = freq_tab;
	printk(KERN_INFO "%s : %d\n", __FUNCTION__, S5PC11X_FREQ_TAB);
...
static ssize_t show_freq_tab(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk(KERN_INFO "%s : %d\n", __FUNCTION__, S5PC11X_FREQ_TAB);

	return sprintf(buf, "%u\n", S5PC11X_FREQ_TAB);
}
static ssize_t store_freq_tab(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned int input;
	int ret;
	...
	s5pc110_change_freq_tab(input);
	
	return size;
}

static DEVICE_ATTR(dvfs_freq_tab, S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH, show_freq_tab, store_freq_tab);

static void dvfs_fs_init()
{
	dvfs_class = class_create(THIS_MODULE, "dvfs");
	if(IS_ERR(dvfs_class))
	{
		printk(KERN_ERR "Failed to create class(dvfs)\n");
	}

	dvfs_fs = device_create(dvfs_class, NULL, 0, NULL, "dvfs_freq_tab");
	if (IS_ERR(dvfs_fs))
		printk(KERN_ERR "Failed to create device(dvfs_fs)!= %ld\n", IS_ERR(dvfs_fs));

	if (device_create_file(dvfs_fs, &dev_attr_dvfs_freq_tab) < 0)
		printk(KERN_ERR "Failed to create device file(%s)!\n", dev_attr_dvfs_freq_tab.attr.name);	
	
	return;
}
#endif // DVFS_SYSFS_FREQ_TAB
...
static int __init s5pc110_cpu_init(struct cpufreq_policy *policy)
{
	...
	dvfs_fs_init();
		...
		S5PC11X_FREQ_TAB = 0;
		S5PC11X_MAXFREQLEVEL = 4;
//		S5PC11X_MAXFREQLEVEL = 2;		// Temporary L2 Limit
		MAXFREQ_LEVEL_SUPPORTED = 5;
		g_dvfs_high_lock_limit = 4;
#endif
