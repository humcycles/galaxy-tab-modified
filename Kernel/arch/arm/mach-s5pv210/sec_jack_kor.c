	...
	if (state)
	{
		...
		if(adc >= 0 && adc <= 400)
		{
			...
			if(HWREV >= 0xE)
			{
				...
				if(HWREV >= 0xF)
				{ // GPIO_MICBIAS2_1 pin is controlled as EAR_MICBIAS to solve the EAR_MICBIAS falling problem when AP chip wakes up
...
//			add_timer(&jack_detect_timer);
			...
			jack_detect_timer_token = 0;
//			schedule_delayed_work(&detect_jack_type_work,200);
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	if (ret < 0)
	{
		...
		det_jack = &hi->port.det_jack; //remove warning...
	...
	if(HWREV >= 0xE)
	{
		if (gpio_is_valid(GPIO_MICBIAS2))
		{
			if (gpio_request(GPIO_MICBIAS2, "MP05"))
				printk(KERN_ERR "Failed to request GPIO_MICBIAS2! \n");
			gpio_direction_output(GPIO_MICBIAS2, 0);
	}
		s3c_gpio_setpull(GPIO_MICBIAS2, S3C_GPIO_PULL_NONE);

		s3c_gpio_slp_cfgpin(GPIO_MICBIAS2, S3C_GPIO_SLP_PREV);
		s3c_gpio_slp_setpull_updown(GPIO_MICBIAS2, S3C_GPIO_PULL_NONE);

		if(HWREV >= 0xF)
		{
			if (gpio_is_valid(GPIO_MICBIAS2_1))
			{
				if (gpio_request(GPIO_MICBIAS2_1, "MP01"))
				printk(KERN_ERR "Failed to request GPIO_MICBIAS2_1! \n");
				gpio_direction_output(GPIO_MICBIAS2_1, 0);
			}
			s3c_gpio_setpull(GPIO_MICBIAS2_1, S3C_GPIO_PULL_NONE);

			s3c_gpio_slp_cfgpin(GPIO_MICBIAS2_1, S3C_GPIO_SLP_PREV);
			s3c_gpio_slp_setpull_updown(GPIO_MICBIAS2_1, S3C_GPIO_PULL_NONE);
		}
